...
export const Experience = () => {
  return (
    <section id="experience" className="w-full py-8 pt-20 border-b border-neutral-800 pb-16 lg:mb-35">
        ...
        {EXPERIENCE.map((exp, index) => (
            <motion.div
                ...
                <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-2">
                    <h3 className="text-xl md:text-2xl font-bold text-neutral-100">{exp.title}</h3>
                    <span className="text-xs px-3 py-1 rounded-full bg-neutral-800 text-stone-300 mt-2 md:mt-0">{exp.duration}</span>
                </div>
                <div className="flex flex-col md:flex-row md:items-center mb-4">
                    <h4 className="text-lg font-medium text-neutral-300">{exp.company}</h4>
                    <span className="md:ml-4 text-sm text-stone-400">{exp.location}</span>
                </div>
